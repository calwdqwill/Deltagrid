...
- GitHub `Deploy Production` при real deploy запускает remote script с `DEPLOY_METHOD=github_actions_ssh`, сохраняет `/tmp/deltagrid-deploy-summary.json` и печатает его в job log; safe-skip по `PROD_*` по-прежнему не создаёт deploy evidence.
- Runbook обновлён production командами для `/version`, `server-smoke`, `release-smoke` и Funding report artifact path `artifacts/production-release/v1.6.0/funding-release-report.json`.
- Изменения не затрагивают backend API, БД, provider calls, trading, execution, route ranking, route selection, route cost bps и diagnostic carry bps.
- Следующая итерация: Funding/Data QA UX batch — улучшить читаемость Funding QA и degraded/empty states без изменения backend/data contracts.

## Обновление 2026-06-20 — v1.6.0 production deploy automation readiness

- `Deploy Production` workflow получил единый шаг `Evaluate production deploy readiness` с `$GITHUB_STEP_SUMMARY` и outputs `deploy_ready`, `deploy_result_status`, `missing_required`.
- Readiness/result статусы теперь явно разделяют `skipped_missing_required_secrets`, `ready_for_real_deploy`, `real_deploy_succeeded` и `real_deploy_failed`.
- Expected production contract вынесен в workflow env и runbook: host `2.25.143.143`, user `root`, app dir `/opt/deltagrid`, deploy key fingerprint `SHA256:TYYi5IayfvNvxRGC3K/J637w8rkUw/+5QtyvtUFJGsg`.
- Workflow не печатает private key, raw secret values, `.env.production` или provider payload; логируются только configured/missing, expected public values и result status.
- `deploy/github-actions-secrets.md`, `README.md` и `RELEASES.md` обновлены под GitHub skipped-vs-real deploy evidence.
- Следующая итерация: production evidence/report hardening — усилить runtime/version proof, `/version` after deploy и funding report production evidence path.
...
- Добавлен release-specific PR template `.github/PULL_REQUEST_TEMPLATE/v1.5.0-funding-qa.md`.
- Handoff получил PR URL с `template=v1.5.0-funding-qa.md`.
- Template фиксирует scope, проверки, safety boundary и release gate после PR.
- Production runtime, deploy workflows, backend API, БД, provider calls и trading/routing capabilities не менялись.
...
- `CI` workflow расширен на push в `codex/**`, чтобы ветка `codex/v1.4.1-funding-release-tooling` могла получить GitHub CI evidence до ручного PR.
- Deploy workflows не расширялись: preview deploy остаётся связан с `preview`, production deploy остаётся связан с `main`/release gate.
- Handoff обновлён: branch CI может быть предварительным evidence, но не заменяет PR review, preview evidence, production backup/deploy, smoke, Browser QA и tag.
- Backend API, БД, provider calls, trading/routing capabilities и production runtime не менялись.
