...
| `GET /api/v1/data/provider-inventory?symbols=BTC,ETH,SOL,HYPE&exchange=okx` | Read-only inventory кандидатов на расширение universe поверх persisted coverage/freshness: `promotion_candidate`, `chart_ready_candidates`, `next_action`, readiness status, 24h/7d summaries и `freshness_scope=requested_symbols` без внешних API-вызовов. |
