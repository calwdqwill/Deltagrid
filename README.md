...
`scripts/funding-release-report.sh` показывает readiness status, total/source rows, frontend marker count, compare status и safety status, но сохраняет exit code исходного smoke.
